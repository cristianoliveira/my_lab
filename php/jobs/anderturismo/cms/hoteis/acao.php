...
@$loc2      = addslashes($_POST["localizacao_mapa"]);
@$status    = $_POST["status"];
...
@$infos     = addslashes($_POST["infos"]);
@$loc1      = $_POST["localizacao_texto"];
@$loc2      = $_POST["pegamapahotel"];
@$loc3      = addslashes($_POST["localizacao_mapa_nova"]);
@$status    = $_POST["status"];
@$imagematual    = $_POST["imagematual"];
	@$imagematual = $_POST['imagematual'];
	
if(empty($loc3)){ $loc3 = $loc2; }

//ver lógica da porra
$update_command = sprintf("UPDATE hoteis SET nome_hotel = '%s', estrelas = '%d', descricao = '%s', informacoes_adicionais = '%s', localizacao = '%s', mapa = '%s', status = '%d' WHERE idhoteis = %d", $nome, $nivel, $descricao, $infos, $loc1, $loc3, $status, $id);
...
if($pega==3)
{
	...
	/* remove galeria */
	$from_delete2 = sprintf("SELECT * FROM imagens WHERE id_imovel = %d", $id);
	$query2 = mysql_query($from_delete2);
	$delete2 = mysql_fetch_array($query2);	
	@unlink('../uploads/imagens/hoteis/' . $delete['img']);
	...
	salvaLog($mensagem);
	$_SESSION['ok'] = "Hotel removido com sucesso!";
	header('Location:listar.php');
