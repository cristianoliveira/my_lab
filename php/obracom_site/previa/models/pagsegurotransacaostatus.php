...
class Model_PagseguroTransacaoStatus extends Model_Padrao
{
	protected $tabela_nome = 'pagseguro_transacao_status';
	protected $colunas = array('id','status');
	protected $colunas_not_null = array('id','status');

	/* 2 propriedades */
	protected $id; // smallint unsigned primary key not null
	protected $status; // varchar(50)

	public function get_id() {return $this->id;}
	public function set_id($integer) {$this->id = ! is_null($integer) ? (int) $integer : NULL;}
	
	public function get_status() {return $this->status;}
	public function set_status($string) {$this->status = ! is_null($string) ? mb_substr(trim($string), 0, 50) : NULL;}
